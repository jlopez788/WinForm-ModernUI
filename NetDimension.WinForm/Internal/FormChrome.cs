...
namespace NetDimension.WinForm
{
    public abstract class FormChrome : Form
    {
        ...
        private void CheckMinimumSize()
        {
            ...
            {
                if (msize.Width > 0)
                    msize.Width = Math.Min(msize.Width, Size.Width);
                if (msize.Height > 0)
                    msize.Height = Math.Min(msize.Height, Size.Height);
        ...
        private void CheckMaximumSize()
        {
            ...
            {
                if (msize.Width > 0)
                    msize.Width = Math.Max(msize.Width, Size.Width);
                if (msize.Height > 0)
                    msize.Height = Math.Max(msize.Height, Size.Height);
        ...
        private void OnUIChangedCore()
        {
            ...
            if ((restoredWinBoundsSpec & BoundsSpecified.Width) != 0 && (restoredWinBoundsSpec & BoundsSpecified.Height) != 0)
                restoreBounds.Size = SizeFromClientSize(restoredWinBounds.Size);
        ...
        protected internal void CalcFormBounds()
        {
            ...
            if (IsMinimizedState(currentBounds))
        ...
        protected internal bool IsLayoutSuspendedCore
        {
            ...
            {
                if (piLayout == null)
                    piLayout = typeof(Control).GetProperty("IsLayoutSuspended", BindingFlags.Instance | BindingFlags.NonPublic);
                if (piLayout != null)
                    return (bool)piLayout.GetValue(this, null);
                return false;
            }
        }
        ...
        protected internal byte LayoutSuspendCountCore
        {
            get
            {
                if (fiLayoutSuspendCount == null)
                    fiLayoutSuspendCount = typeof(Control).GetField("layoutSuspendCount", BindingFlags.Instance | BindingFlags.NonPublic);
                if (fiLayoutSuspendCount != null)
                    return (byte)fiLayoutSuspendCount.GetValue(this);
                return 1;
            }
        }
        ...
        private bool isMaximizedTest = false;
        ...
        #endregion Windows Message Handlers
